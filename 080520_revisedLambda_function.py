        ...
        if age is not None:
            if age >= 65:
                return intent_request(
                    False,
                    "age",
                    "{first_name}, you should be younger than 65 years old to use this service, "
                    "please provide a different date of birth.",
                )
        if investment_amount is not None:
            # Since parameters are strings it's important to cast values
            investment_amount = float(investment_amount)
            if investment_amount >= 0 and investment_amount < 5000:
                return intent_request(
                    False,
                    "investmentAmount",
                    "{first_name}, the amount to convert should be at least $5,000, "
                    "please provide a correct amount in USD to convert.",
                )

        # A True results is returned if age or amount are valid
        return intent_request(True, None, None)
...
def risk_recommendation(risk_level):
    if risk_level == 'none':
        intitial_recommendaiton = '100% bonds (AGG), 0% equities (SPY)'
